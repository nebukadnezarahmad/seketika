...
export function KelolaMenu() {
  ...
  /* `undefined` berarti fotonya tidak disentuh selama lembar terbuka, `null` berarti pedagang mengembalikannya ke foto bawaan, teks berarti ada foto baru yang menunggu disimpan. */
  const [fotoBaru, setFotoBaru] = React.useState<string | null | undefined>(
    undefined,
  );
  ...
  /* `sunting` yang null berarti sedang membuat menu baru; formulirnya sama, hanya judul dan tombolnya yang berbeda kata. */
  ...
  /* Foto yang sedang ditampilkan di lembar: unggahan yang belum disimpan kalau ada, kalau dikembalikan ke bawaan pakai bawaannya, selebihnya foto yang berlaku sekarang. */
  ...
  const fotoLembar =
    fotoBaru === undefined
      ...
      : (fotoBaru ?? fotoBawaan);
  /* Tombol "kembalikan foto bawaan" hanya masuk akal kalau memang ada foto sendiri yang sedang dipakai atau baru saja dipilih. */
  const adaFotoSendiri =
    fotoBaru !== null &&
    (fotoBaru !== undefined || Boolean(fotoUnggahan[idSunting]));
        ...
        >
          <div className="px-4 pb-6 pt-1">
            ...
            <div className="mt-4">
              ...
              {salahFoto && (
                <p className="mt-1.5 text-[11px] text-merah">{salahFoto}</p>
              )}
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  /* Nilainya dikosongkan supaya memilih berkas yang sama dua kali berturut-turut tetap memicu peristiwanya. */
                  e.target.value = "";
                }}
            ...
            <label className="mt-3 block">
              ...
              />
              {salah.nama && (
                <span className="mt-1 block text-[11px] text-merah">
                  {salah.nama}
                </span>
              )}
            </label>
            ...
            <label className="mt-3 block">
              ...
              {/* Kolomnya angka lewat papan ketik telepon, bukan `type="number"`: pemisah ribuan yang diketik pengguna akan ditolak diam-diam oleh kolom angka, dan harga rupiah nyaris selalu ditulis dengan titik. */}
              <input
                value={harga}
                onChange={(e) =>
                  setHarga(harganRapi(angkaHarga(e.target.value)))
                }
                inputMode="numeric"
                placeholder="15.000"
            ...
            {!menambah && (
              <div className="mt-3">
                {konfirmasiHapus ? (
                  /* Penghapusan minta ketukan kedua. */
                  <div className="rounded-[16px] border border-merah/30 bg-merah-lembut p-3.5">
                    <p className="text-[12.5px] leading-snug text-merah">
                      Hapus{" "}
                      <strong className="font-bold">{sunting?.nama}</strong>{" "}
                      dari daftar menu? Kalau cuma kehabisan bahan, matikan saja
                      sakelarnya.
                    </p>
    ...
    >
      ...
      <div className="px-4 pb-6 pt-3">
        <p className="rounded-[16px] border border-garis bg-white px-3.5 py-3 text-[12px] leading-relaxed text-tinta-3">
          Ketuk menu untuk mengubah nama, keterangan, atau harganya. Sakelar di
          kanan menyembunyikannya sementara dari aplikasi warga.
        </p>
          ...
          {daftar.map((m) => {
            ...
            return (
              ...
              >
                ...
                {/* Sakelar berupa input sungguhan yang disembunyikan dari mata dan dibungkus label. */}
