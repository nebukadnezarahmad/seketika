...
test.describe("Pintu masuk", () => {
  test("sambutan menerangkan aplikasinya, bukan cuma dua tombol", async ({
    page,
  }) => {
    await page.goto("/sambutan");

    await expect(
      page.getByRole("heading", { name: /Panggil jajanan/ }),
    ).toBeVisible();
    await expect(page.getByText("Mau ngapain hari ini?")).toBeVisible();

    /* Dua keping cara memesan. */
    const keping = page.getByRole("listitem");
    await expect(
      keping.filter({ hasText: "Panggil ke lokasimu" }),
    ).toBeVisible();
    ...
    /* Ilustrasinya karya tim di berkas Figma, bukan hiasan yang bisa hilang tanpa akibat: tanpa itu layar pertama cuma teks dan dua tombol. */
    /* Dicari lewat sumbernya, bukan peran gambar: alt-nya sengaja dikosongkan karena isinya sudah diucapkan teks di sekitarnya. */
  ...
  /* Pada ekspor Figma aslinya pasangan roda berpusat 8,5 unit di kanan batang gerobaknya, dan seluruh lambang terbaca miring. */
  test("gerobak pada lambang lurus, bukan miring ke satu sisi", async ({
    page,
  }) => {
    await page.goto("/sambutan");
    ...
    if (!batang || !rodaKiri || !rodaKanan)
      throw new Error("bagian lambang tidak terukur");

    const pusat = (k: { x: number; width: number }) => k.x + k.width / 2;
    expect(
      Math.abs(pusat(rodaKiri) + pusat(rodaKanan) - 2 * pusat(batang)),
    ).toBeLessThan(1.5);
  });
  ...
  test("kata sandi bisa ditampilkan dan disembunyikan lagi", async ({
    page,
  }) => {
    await page.goto("/daftar");
  ...
  test("Masuk mengantar pengguna lama langsung ke berandanya", async ({
    page,
  }) => {
