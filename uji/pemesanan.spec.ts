...
/** Layar menu tidak lagi memakai keranjang bertahap. */
test.describe("Pemesanan", () => {
  ...
  test("daftar menu tidak lagi punya tombol tambah", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: /Panggil Penjual/ }).first(),
    ).toBeVisible();
  });

  test("mengetuk menu menaikkan lembar berisi keterangan lengkapnya", async ({
    page,
  }) => {
    await page.goto("/pedagang/bakso-pak-anton/menu");
    ...
    await expect(
      lembar.getByRole("heading", { name: "Bakso Mercon" }),
    ).toBeVisible();
    await expect(
      lembar.getByText("Bakso isian cabe rawit ekstra pedas"),
    ).toBeVisible();
    await expect(lembar.getByText("Rp 15.000")).toBeVisible();
    await expect(
      lembar.getByRole("button", { name: /Panggil Penjual/ }),
    ).toBeVisible();
  ...
  test("panggil penjual dari lembar membawa menunya sebagai ancar-ancar", async ({
    page,
  }) => {
    await page.goto("/pedagang/bakso-pak-anton/menu");
    ...
    await page
      .getByRole("dialog")
      .getByRole("button", { name: /Panggil Penjual/ })
      .click();
  ...
  test("panggil penjual dari atas daftar memesan tanpa rincian", async ({
    page,
  }) => {
    await page.goto("/pedagang/bakso-pak-anton/menu");
    await page
    ...
    await page.waitForURL(/\/pesanan\/ord-\d+/);
    /* Pesanan tanpa rincian tidak boleh terbaca seperti pesanan gagal; layarnya harus menerangkan kenapa daftarnya kosong. */
    await expect(page.getByText(/menunya dipilih/)).toBeVisible();
    await expect(page.getByText("Dihitung di tempat")).toBeVisible();
  });

  test("penyaring pada daftar pesanan memilah menurut status", async ({
    page,
  }) => {
    await page.goto("/pesanan");
    await expect(page.getByRole("link", { name: /Lihat Detail/ })).toHaveCount(
      4,
    );
