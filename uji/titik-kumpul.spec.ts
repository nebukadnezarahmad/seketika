...
/**
 * Titik Kumpul adalah fitur yang membedakan SEKETIKA dari aplikasi
 * pesan-antar biasa, jadi alurnya diuji paling rinci.
 */
test.describe("Titik Kumpul", () => {
  ...
  test("sekali bergabung tidak terhitung dua kali", async ({ page }) => {
    ...
    /* Tombolnya berganti jadi keterangan menunggu, jadi tidak ada jalan
       untuk menambah diri sendiri lagi. */
    await expect(page.getByRole("button", { name: /Ikut Pesan Sekarang/ })).toHaveCount(0);
    await page.reload();
    await expect(page.getByText("4/5 warga")).toBeVisible();
  });

  test("target terpenuhi mengubah ajakan jadi arahan ke lokasi", async ({ page }) => {
  ...
  test("membuat titik kumpul baru dan mendapat tautan undangan", async ({ page }) => {
    await page.goto("/kolab/buat?pedagang=siomay-pak-agus");

    const publikasikan = page.getByRole("button", { name: /Publish Titik Kumpul/ });
    await expect(publikasikan).toBeDisabled();

    await page.getByPlaceholder("Misal: RT 03 Perumahan Indah").fill("RT 09 Bukit Indah");
    ...
    await expect(page.getByRole("heading", { name: /Berhasil Dibuat/ })).toBeVisible();
    await expect(page.getByText(/\/kolab\/tk-/)).toBeVisible();
  });

  test("titik kumpul buatan sendiri muncul di daftar sebagai milik Anda", async ({ page }) => {
    await page.goto("/kolab/buat?pedagang=siomay-pak-agus");
    await page.getByPlaceholder("Misal: RT 03 Perumahan Indah").fill("RT 09 Bukit Indah");
    await page.getByRole("button", { name: "Tambah target" }).click();
    await page.getByRole("button", { name: "Tambah target" }).click();
    ...
    const kartu = page.getByRole("listitem").filter({ hasText: "RT 09 Bukit Indah" });
