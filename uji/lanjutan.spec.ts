 ...
 *
 * Beberapa keadaan tidak bisa dicapai lewat pemakaian biasa dalam waktu
 * satu uji — tenggat titik kumpul, misalnya, baru lewat berjam-jam
 * kemudian. Menyuntik penyimpanan lebih jujur daripada memundurkan jam
 * peramban, karena yang diuji tetap kode yang sama dengan yang dipakai
 * pengguna, hanya titik berangkatnya yang digeser.
 */
async function ubahSimpanan(page: Page, ubah: (keadaan: Simpanan) => void) {
  await page.evaluate((sumber) => {
    ...
    /* Fungsinya diseberangkan sebagai teks lalu dirakit ulang di dalam
       peramban. `page.evaluate` hanya bisa mengirim nilai yang bisa
       diserialkan, dan fungsi bukan salah satunya. */
...
test.describe("Menolak pesanan", () => {
  test("pesanan yang ditolak tidak terhitung sebagai pemasukan", async ({ page }) => {
    ...
    /* Inti perbaikannya: menolak tidak boleh menaikkan omzet sepeser pun.
       Sebelum ini tombol tolak menyetel status selesai, dan Buku Kas ikut
       menghitungnya sebagai pemasukan Rp 105.000. */
    await page.goto("/d/rekap");
    expect(angka(await nominal.innerText())).toBe(sebelum);
  });

  test("pesanan yang ditolak punya kelompoknya sendiri, bukan menumpang selesai", async ({ page }) => {
    await lewatiPengenalan(page, "pedagang", "Pak Anton");
    await page.getByRole("listitem").filter({ hasText: "Pak Dedi" })
      .getByRole("button", { name: "Tolak" }).click();
...
test.describe("Kelola menu pedagang", () => {
  test("mematikan menu menyembunyikannya dari warga", async ({ page }) => {
    ...
    await page.getByRole("checkbox", { name: "Tampilkan Bakso Mercon" }).uncheck({ force: true });
    ...
    await expect(page.getByRole("button", { name: /Bakso Mercon/ })).toHaveCount(0);
    await expect(page.getByRole("button", { name: /Bakso Komplit/ })).toBeVisible();
  });

  test("tautan kelola menu tidak lagi mendarat di layar pembeli", async ({ page }) => {
    await lewatiPengenalan(page, "pedagang", "Pak Anton");
    await page.goto("/d/profil");
    await page.getByRole("link", { name: /Kelola Menu/ }).click();

    await page.waitForURL("**/d/menu");
    /* Dulu tautan ini mendarat di daftar menu warga, yang kini berisi
       tombol memanggil penjual: pedagang ditawari memanggil gerobaknya
       sendiri. */
    await expect(page.getByRole("button", { name: /Panggil Penjual/ })).toHaveCount(0);
  });
});

test.describe("Penilaian", () => {
  test("bintang hanya ditawarkan pada pesanan yang sudah selesai", async ({ page }) => {
  ...
  test("nilai yang sudah dikirim tidak bisa diubah lagi", async ({ page }) => {
    ...
    await page.getByRole("radio", { name: /^4 bintang/ }).check({ force: true });
    ...
    await expect(page.getByRole("button", { name: "Kirim Penilaian" })).toHaveCount(0);
...
test.describe("Obrolan titik kumpul", () => {
  test("pesan terkirim, tersimpan, dan membawa nama pengirimnya", async ({ page }) => {
    await lewatiPengenalan(page, "pembeli", "Dewi");
    ...
    await lembar.getByLabel("Tulis pesan untuk warga lain").fill("Kumpul jam 5 ya");
...
test.describe("Kedaluwarsa titik kumpul", () => {
  test("titik kumpul yang lewat tenggat berhenti menerima warga", async ({ page }) => {
    await lewatiPengenalan(page, "pembeli", "Dewi");
    await page.goto("/kolab/tk-01");
    await expect(page.getByRole("button", { name: /Ikut Pesan Sekarang/ })).toBeVisible();
    ...
    /* Pada daftar pun ia tidak lagi menawarkan bergabung, tapi juga tidak
       lenyap begitu saja; menghilang diam-diam membuat orang mengira
       datanya rusak. */
    await page.goto("/kolab");
    const kartu = page.getByRole("listitem").filter({ hasText: "RT 05 Blok C" });
...
test.describe("Pusat notifikasi", () => {
  ...
  test("pedagang melihat kabar yang berbeda dari warga", async ({ page }) => {
    ...
    await expect(page.getByText("Permintaan titik kumpul").first()).toBeVisible();
