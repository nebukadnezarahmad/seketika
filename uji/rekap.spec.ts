...
/** Buku Kas membuka angka yang selama ini sudah tersimpan tapi tidak pernah dijumlahkan. */
test.describe("Buku Kas", () => {
  ...
  test("menampilkan pendapatan hari ini dan grafik tujuh hari", async ({
    page,
  }) => {
    await expect(page.getByRole("heading", { name: "Buku Kas" })).toBeVisible();
    await expect(page.getByText("Pendapatan Hari Ini")).toBeVisible();

    /* Data contoh menyelesaikan dua pesanan hari ini, jadi angkanya tidak boleh nol rupiah. */
    ...
    await expect(
      page.getByRole("heading", { name: /Tujuh Hari Terakhir/ }),
    ).toBeVisible();
    /* Tujuh batang, tiap batang membawa nominalnya sendiri untuk pembaca layar; tanpa itu grafiknya tidak berarti apa-apa bagi yang tidak melihatnya. */
    const batang = page
      .locator("section", { hasText: "Tujuh Hari Terakhir" })
      .getByRole("listitem");
    await expect(batang).toHaveCount(7);
    await expect(batang.last()).toContainText("(hari ini)");

    await expect(
      page.getByRole("heading", { name: /Menu Terlaris/ }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: /Jam Paling Ramai/ }),
    ).toBeVisible();
    await expect(page.getByText(/Paling ramai jam/)).toBeVisible();
  });

  test("menyelesaikan satu pesanan menaikkan angka pendapatan", async ({
    page,
  }) => {
    const nominal = page.locator("section").first().getByText(/^Rp /).first();
    const sebelum = await nominal.innerText();

    /* Pak Dedi memesan Bakso Komplit 25.000 kali tiga dan Bakso Mercon 15.000 kali dua, jadi menyelesaikannya menambah 105.000. */
  ...
  test("langganan Pro bisa dinyalakan dan dimatikan lagi", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /Laporan 30 Hari/ }),
    ).toHaveCount(0);
    ...
    await expect(
      page.getByRole("heading", { name: /Prakiraan Kawasan/ }),
    ).toBeVisible();

    /* Dimatikan lagi, isinya ikut menutup. */
  ...
  test("catatan stok terkunci sebelum langganan menyala", async ({ page }) => {
    ...
    await page
      .getByRole("button", { name: "Tambah stok Bakso Komplit" })
      .click();
    await page
      .getByRole("button", { name: "Tambah stok Bakso Komplit" })
      .click();
  ...
  test("dijangkau dari lembar pesanan hari ini dan dari halaman toko", async ({
    page,
  }) => {
    await page.goto("/d");
